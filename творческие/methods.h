...
{
    for (int i = 1; i < 6; ++i) graph.insert_vertex(i);
...
template<class T>
void fill_graph(Graph<T>& graph)
{
    int vertex, edge_weight, from_vertex, to_vertex;
...
template<class T>
void Graph<T>::insert_vertex_full(const T& new_vertex)
{
    int size = get_count_vertex();
    vertex_container.push_back(new_vertex);
    count_of_vertex++;
    vector<int> new_vect(size + 1);
    ...
    for (int i = 0; i < size; ++i)
    {
        if (i != new_vertex)
        {
            ...
            do
            {
                cout << "Input weight from " << new_vertex << " vertex to " << i + 1 << " vertex: "; cin >> value;
    ...
    adjacency_matrix.push_back(new_vect);
}

template<class T>
int Graph<T>::delete_vertex(const T& to_delete)
{
    int index = get_vertex_index(to_delete);
    if (index == -1) return -1;

    vector<T>::iterator iterator = vertex_container.begin();

    for (int i = 0; i < index; ++i)
    {
        ++iterator;
    }
    vertex_container.erase(iterator);

    auto it = adjacency_matrix.begin();
    for (int i = 0; i < index; ++i)
    {
        ++it;
    }
    adjacency_matrix.erase(it);

    for (int i = 0; i < count_of_vertex - 1; ++i)
    {
        auto it = adjacency_matrix[i].begin();
        for (int j = 0; j < index; ++j) it++;
        adjacency_matrix[i].erase(it);
    }
